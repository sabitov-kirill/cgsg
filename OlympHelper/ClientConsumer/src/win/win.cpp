 ...
 *       HINSTANCE hInstance;
 */
win::win( HINSTANCE hInstance, const char *Title, BOOL IsShowed ) : 
  hWnd(), W(100), H(100), hInstance(hInstance),
  IsInit(false), IsFullScreen(false), FullScreenSaveRect(), MouseWheel(0), IsActive(false)
{
  const char WND_CLASS_NAME[] = "SearchRequestProgrammClass";
  ...
  hWnd = CreateWindow(
    WND_CLASS_NAME,
    Title,
    WS_OVERLAPPEDWINDOW | (IsShowed ? WS_VISIBLE : 0),
                               ...
                               WPARAM wParam, LPARAM lParam )
{
  ...
  default:
    ...
    if (Win != nullptr)
      ...
      case WM_HOTKEY:
        Win->OnHotkey((INT)wParam, (UINT)LOWORD(lParam), (UINT)HIWORD(lParam));
        return 0;

      case WM_CHAR:
        Win->OnKeyboardClick((INT)wParam, (UINT)lParam);
        return 0;
